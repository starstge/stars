...
async def ensure_db_pool(max_retries: int = 3, retry_delay: float = 1.0) -> asyncpg.Pool:
    global _db_pool
        ...
        if _db_pool is None or _db_pool._closed:
            try:
                ...
                _db_pool = await asyncpg.create_pool(
                    ...
                    min_size=1,
                    max_size=5,  # Reduced for Render free/starter plans
                    max_queries=50000,
                    max_inactive_connection_lifetime=300,
                    timeout=30,
                    command_timeout=60
                )
                # Validate pool by running a simple query
            ...
            try:
                ...
                _db_pool = None  # Force recreation
                if attempt + 1 < max_retries:
                    await asyncio.sleep(retry_delay)
                continue
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Initialize tech_break_info in bot_data if not present
    ...
    try:
        async with (await ensure_db_pool()) as conn:
            ...
            if data == "profile":
                ...
                external_id = f"order{int(datetime.now(pytz.UTC).timestamp() * 1000)}"
                # Store order in bot_data
                if "orders" not in context.bot_data:
                    context.bot_data["orders"] = {}
                context.bot_data["orders"][external_id] = {
                    "user_id": user_id,
                    "recipient": recipient,
                    "stars_amount": stars,
                    "price_ton": price_ton
                ...
                payment_link = f"ton://transfer/{OWNER_WALLET}?amount={int(price_ton * 1e9)}&text={external_id}"
                text = (
                    f"Подтвердите покупку:\n"
                    f"Звезды: {stars}\n"
                    f"Получатель: {recipient}\n"
                    f"Сумма: {price_ton:.2f} TON\n"
                    f"Кошелек для оплаты: <code>{OWNER_WALLET}</code>\n"
                    f"Комментарий к переводу: <code>{external_id}</code>\n"
                    f"Переведите указанную сумму на кошелек, указав комментарий."
                )
                keyboard = [
                    [InlineKeyboardButton("Оплатить (Tonkeeper)", url=payment_link)],
                    [InlineKeyboardButton("Проверить оплату", callback_data=f"check_payment_manual_{external_id}")],
                    [InlineKeyboardButton("🔙 Назад", callback_data="buy_stars")]
                ]
                ...
                await conn.execute(
                    ...
                    user_id, recipient, stars, price_ton, external_id, datetime.now(pytz.UTC), "not_checked"
                )
                current_text = query.message.text
                current_reply_markup = query.message.reply_markup
                new_reply_markup = InlineKeyboardMarkup(keyboard)
                ...
                await query.answer()
                context.user_data["state"] = 28  # New state for payment menu
                context.user_data["invoice_id"] = external_id
                context.user_data["price_ton"] = price_ton
                await log_analytics(user_id, "pay_stars_menu", {"stars": stars, "recipient": recipient, "invoice_id": external_id})
                return 28

            elif data.startswith("check_payment_manual_"):
                external_id = data.split("_")[-1]
                stars = context.user_data.get("stars_amount")
                recipient = context.user_data.get("recipient")
                price_ton = context.user_data.get("price_ton")
                if not stars or not recipient or not price_ton or external_id not in context.bot_data.get("orders", {}):
                    ...
                    context.user_data["state"] = 5
                    return 5
                # Check payment via TON API
                async with aiohttp.ClientSession() as session:
                    try:
                        url = f"https://tonapi.io/v2/blockchain/accounts/{OWNER_WALLET}/transactions?limit=20"
                ...
                if "processed_txs" not in context.bot_data:
                    context.bot_data["processed_txs"] = set()
                for tx in txs:
                    tx_hash = tx.get("hash")
                    if tx_hash in context.bot_data["processed_txs"]:
                        continue
                    msg = tx.get("in_msg")
                    if not msg:
                        continue
                    comment = (
                        msg.get("decoded_body", {}).get("text")
                        or msg.get("message")
                        or msg.get("raw_body")
                        or ""
                    )
                    amount = float(msg.get("value", 0)) / 1e9
                    if comment == external_id and abs(amount - price_ton) < 0.01:  # Allow small discrepancy
                        context.bot_data["processed_txs"].add(tx_hash)
                        # Update transaction status
                        await conn.execute(
                            "UPDATE transactions SET checked_status = $1, purchase_time = $2 WHERE invoice_id = $3",
                            "confirmed", datetime.now(pytz.UTC), external_id
                        )
                        # Remove order
                        context.bot_data["orders"].pop(external_id, None)
                        text = f"Спасибо за покупку!\n{stars} звезд для {recipient} будут начислены после проверки."
                        current_text = query.message.text
                        current_reply_markup = query.message.reply_markup
                        new_reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="back_to_menu")]])
                        if current_text != text or str(current_reply_markup) != str(new_reply_markup):
                        ...
                        await log_analytics(user_id, "payment_confirmed_manual", {"stars": stars, "recipient": recipient, "invoice_id": external_id})
                        return 0
                # Payment not found
                await query.answer("Платеж еще не получен. Попробуйте снова.", show_alert=True)
                return context.user_data["state"]
                ...
                if not transactions:
                    ...
                    for idx, t in enumerate(transactions, start=1 + offset):
                        ...
                        eest_time = utc_time.astimezone(pytz.timezone('Europe/Tallinn')).strftime('%Y-%m-%d %H:%M:%S EEST')
                        try:
                            tg_user = await context.bot.get_chat(t['user_id'])
                            username = f"@{tg_user.username}" if tg_user.username else f"ID {t['user_id']}"
                        except TelegramError:
                            username = await conn.fetchval("SELECT username FROM users WHERE user_id = $1", t['user_id'])
                            username = f"@{username}" if username else f"ID {t['user_id']}"
                        text += (
                            f"{idx}. {username} купил {t['stars_amount']} звезд для {t['recipient_username']} "
                            f"за {t['price_ton']:.2f} TON в {eest_time}\n\n"
                        )
                keyboard = []
                if total_transactions > (page + 1) * transactions_per_page:
                ...
                await query.answer()
                context.user_data["state"] = 27
                await log_analytics(user_id, "view_admin_transactions", {"page": page})
                return 27

            elif data.startswith("admin_transactions_") and is_admin:
                ...
                for user in users:
                    try:
                        tg_user = await context.bot.get_chat(user['user_id'])
                        username = f"@{tg_user.username}" if tg_user.username else f"ID <code>{user['user_id']}</code>"
                        # Update username in database if it has changed
    ...
    try:
        async with (await ensure_db_pool()) as conn:
            ...
            if state == STATES["buy_stars_recipient"]:
                ...
                context.user_data["edit_user_id"] = edit_user_id
                try:
                    tg_user = await context.bot.get_chat(edit_user_id)
                    username = f"@{tg_user.username}" if tg_user.username else f"ID {edit_user_id}"
                    # Update username in database if changed
                    if tg_user.username and tg_user.username != user['username']:
                        await conn.execute(
                            "UPDATE users SET username = $1 WHERE user_id = $2",
                            tg_user.username, edit_user_id
                        )
                except TelegramError:
                    username = f"@{user['username']}" if user['username'] else f"ID {edit_user_id}"
                reply_text = (
